...
public:
    ...
    CAmount contractFee;
    CTransaction contactTx;
    CTransaction refundTx;
    CAmount refundFee;
};
...
{
    MutableTransactionSignatureCreator creator(pwallet, &refundTx, 0, SIGHASH_ALL);
    std::vector<unsigned char> vch;
    CPubKey pubKey;
    if (!creator.CreateSig(vch, address, pubKey, contractRedeemscript)) {
        return false;
    }
    ...
    return VerifyScript(refundSigScript, contractPubKeyScript, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker());
}
...
{
    MutableTransactionSignatureCreator creator(pwallet, &redeemTx, 0, SIGHASH_ALL);
    std::vector<unsigned char> vch;
    CPubKey pubKey;
    if (!creator.CreateSig(vch, address, pubKey, contractRedeemscript)) {
        return false;
    ...
    return VerifyScript(redeemSigScript, contractPubKeyScript, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker());
}
...
void BuildRefundTransaction(CWallet* pwallet, const CScript& contractRedeemscript, const CMutableTransaction& contractTx, CMutableTransaction& refundTx, CAmount& refundFee)
{
    ...
    CBitcoinAddress bitcoinAddress = GetRawChangeAddress(pwallet);
    CTxDestination refundAddress = bitcoinAddress.Get();
    ...
    refundFee = CWallet::GetMinimumFee(refundTxSize, nTxConfirmTarget, ::mempool);
...
void BuildContract(SwapContract& contract, CWallet* pwallet, CKeyID dest, CAmount nAmount, int64_t locktime, std::vector<unsigned char> secretHash)
{
    ...
    CBitcoinAddress refundAddress = GetRawChangeAddress(pwallet);

    CKeyID refundKeyId;
    refundAddress.GetKeyID(refundKeyId);

    CScript contractRedeemscript = CreateAtomicSwapRedeemscript(refundKeyId, dest, locktime, SECRET_SIZE, secretHash);
    CScriptID swapContractAddr = CScriptID(contractRedeemscript);
    CScript contractPubKeyScript = GetScriptForDestination(swapContractAddr);
    ...
    CAmount refundFee;
    BuildRefundTransaction(pwallet, contractRedeemscript, CMutableTransaction(wtx), refundTx, refundFee);

    contract.contractRedeemscript = contractRedeemscript;
    contract.contractAddr = swapContractAddr;
    contract.contactTx = wtx;
    contract.contractFee = nFeeRequired;
    contract.refundTx = refundTx;
...
UniValue SwapTxToUniv(const CTransaction& tx)
{
    UniValue res(UniValue::VOBJ);
    res.push_back(Pair("txid", tx.GetHash().GetHex()));
    res.push_back(Pair("hex", EncodeHexTx(tx)));
    return res;
}
...
void SwapContractToUniv(const SwapContract& contract, UniValue& res)
{
    ...
    c.push_back(Pair("address", CBitcoinAddress(contract.contractAddr).ToString()));
...
UniValue initiateswap(const UniValue& params, bool fHelp)
{
    ...
    CTxDestination dest = CBitcoinAddress(params[0].get_str()).Get();
    if (dest.which() != 1) {
...
UniValue auditswap(const UniValue& params, bool fHelp)
{
    ...
    std::vector<unsigned char> scriptData(ParseHexV(params[0], "hexscript"));
    ...
    UniValue res(UniValue::VOBJ);
    res.push_back(Pair("contractAddress", CBitcoinAddress(contractAddr).ToString()));
    res.push_back(Pair("contractValue", ValueFromAmount(tx.vout[contractOutIndex].nValue)));
    res.push_back(Pair("recipientAddress", CBitcoinAddress(recipient).ToString()));
    res.push_back(Pair("refundAddress", CBitcoinAddress(refundAddr).ToString()));
    res.push_back(Pair("secretHash", HexStr(secretHash.begin(), secretHash.end())));
...
UniValue participateswap(const UniValue& params, bool fHelp)
{
    ...
    std::vector<unsigned char> secretHash(ParseHexV(params[2], "secrethash"));
...
UniValue redeemswap(const UniValue& params, bool fHelp)
{
    CWallet * const pwallet = pwalletMain;
    if (!EnsureWalletIsAvailable(fHelp)) {
        return NullUniValue;
    ...
    std::vector<unsigned char> contract(ParseHexV(params[0], "hexscript"));

    CTransaction contractTx;
    if (!DecodeHexTx(contractTx, params[1].get_str())) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Failed to decode contract transaction");
    }

    std::vector<unsigned char> secret(ParseHexV(params[2], "secret"));
    ...
    CBitcoinAddress bitcoinAddr = GetRawChangeAddress(pwallet);
    CTxDestination addr = bitcoinAddr.Get();
    ...
    CAmount redeemFee = CWallet::GetMinimumFee(redeemTxSize, nTxConfirmTarget, ::mempool);
    redeemTx.vout[0].nValue = contractTx.vout[contractOutPoint.n].nValue - redeemFee;
    if (redeemTx.vout[0].IsDust(::minRelayTxFee)) {
    ...
    UniValue res = SwapTxToUniv(redeemTx);
    res.push_back(Pair("Redeem fee", ValueFromAmount(redeemFee)));

    return res;
}

UniValue extractsecret(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
    ...
    std::vector<unsigned char> secretHash(ParseHexV(params[1], "secrethash"));
    if (secretHash.size() != CSHA256::OUTPUT_SIZE) {
         throw JSONRPCError(RPC_TYPE_ERROR, "Secret hash has wrong size");
    }
...
UniValue refundswap(const UniValue& params, bool fHelp)
{
    ...
    UniValue res = SwapTxToUniv(refundTx);
